...
//	得到cookie
	var str = getCookie("person");
	var persons = JSON.parse(str);
	document.getElementById("welcome").innerHTML = "hi~欢迎来到网酒网"+persons.userName;
};
...
counter = function(){
	...
	d = new Date();
	endtime = d.setHours(22,00,00);
	left =  parseInt(endtime - now);
	leftHour = Math.floor(left / (60*60*1000));
	leftMin = Math.floor(left / ( 60*1000) - leftHour*60);
	leftSec = left / 1000 - leftHour*60*60 - leftMin*60;
	show.innerHTML = +z(leftHour)+":"+ z(leftMin)+":"+ z(leftSec) ;
};
counter();
